...
test_artifact_root="${FOODMAPPER_TEST_ARTIFACT_ROOT:-$temporary_root}"
test_artifact_root="${test_artifact_root%/}"
case "$test_artifact_root" in
    /*) ;;
    *)
        printf '%s\n' "FOODMAPPER_TEST_ARTIFACT_ROOT must be an absolute path." >&2
        exit 1
        ;;
esac
[ "$test_artifact_root" != "/" ] || {
    printf '%s\n' "FOODMAPPER_TEST_ARTIFACT_ROOT cannot be the filesystem root." >&2
    exit 1
}
test_id="$(uuidgen | tr '[:upper:]' '[:lower:]')"
test_root="${test_artifact_root}/foodmapper-xctest-${test_id}"
derived_data="${test_artifact_root}/foodmapper-derived-data-${test_id}"
preferences_root="${test_artifact_root}/foodmapper-preferences-${test_id}"
...
require_xcode_26_6() {
    ...
    case "$DEVELOPER_DIR" in
        ...
        *)
            printf '%s\n' "The isolated test suite requires Xcode 26.6 from /Applications/Xcode.app or /Applications/Xcode_26.6.app." >&2
            return 1
            ;;
    esac
