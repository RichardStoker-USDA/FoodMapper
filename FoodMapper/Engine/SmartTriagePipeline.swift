...
private let logger = Logger(subsystem: "com.foodmapper", category: "smart-triage-pipeline")

/// Review-optimized matching pipeline: Qwen3-Embedding retrieval + Qwen3-Reranker scoring.
///
/// Similar to TwoStagePipeline but defaults to top-10 candidate retrieval for the review
/// workflow. The reranker provides well-calibrated P(yes) scores that feed directly
/// into the three-zone triage system (auto-accept / pending / auto-reject).
///
/// Stage 1: Qwen3-Embedding (instruction-aware) retrieves top-K candidates per input
/// Stage 2: Qwen3-Reranker (batch prefill) scores all candidates per input
final class SmartTriagePipeline: MatchingPipelineProtocol {
    let pipelineType: PipelineType = .qwen3SmartTriage
    var name: String { pipelineType.displayName }

    private let engine: MatchingEngine
    private let reranker: QwenRerankerModel
    private let configuredTopK: Int

    init(engine: MatchingEngine, reranker: QwenRerankerModel, topK: Int = 10) {
        self.engine = engine
        self.reranker = reranker
        self.configuredTopK = topK
    }
    ...
    func match(
        ...
        // Set custom instruction for asymmetric Qwen3-Embedding
        await engine.setInstruction(instruction)

        logger.info("[Pipeline] SmartTriage | Stage: embedding | Instruction: \(instruction?.prefix(100) ?? "(model default)")")
        logger.info("[Pipeline] SmartTriage | Stage: reranker | Instruction: \(rerankerInstruction?.prefix(100) ?? "(model default)")")

        // Stage 1: Embedding retrieval (70% of progress)
        let stage1Start = Date()
        onPhaseChange?(.loadingDatabase)
        logger.info("Stage 1: Retrieving top-\(self.configuredTopK) candidates for \(totalInputs) inputs")
        ...
        logger.info("Stage 2: Reranking \(totalInputs) inputs (top-\(self.configuredTopK) candidates each)")
        ...
        for (inputIndex, candidates) in topKResults.enumerated() {
            ...
            // Rerank candidates via batch prefill
            ...
            // Build candidates sorted by reranker score descending
